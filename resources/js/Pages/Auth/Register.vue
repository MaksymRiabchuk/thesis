<script setup lang="ts">
import { Link, useForm } from '@inertiajs/vue3';
import { route } from 'ziggy-js';
import GuestLayout from '@/Layouts/GuestLayout.vue';
import { Input } from '@/components/ui/input';
import { Label } from '@/components/ui/label';
import { Button } from '@/components/ui/button';
import { watch } from 'vue';
import InputErrorMessage from "@/components/Admin/InputErrorMessage.vue";
import {RegisterForm} from "@/types/auth/register";

defineOptions({
  layout: GuestLayout,
});

const form = useForm<RegisterForm>({
  email: '',
  name: '',
  password: '',
  password_confirmation:'',
});

const onSubmit = async (): Promise<void> => {
  form.post(route('auth.sign.up'));
};

watch(
    () => form.email,
    () => {
      form.clearErrors('email');
    }
);

watch(
    () => form.name,
    () => {
      form.clearErrors('name');
    }
);

watch(
    () => form.password,
    () => {
      form.clearErrors('password');
    }
);
watch(
    () => form.password_confirmation,
    () => {
      form.clearErrors('password_confirmation');
    }
);
</script>

<template>
  <div class="w-full max-w-md flex flex-col items-center rounded-lg border border-gray-200 bg-white p-6 shadow-lg">
    <h2 class="mb-6 text-2xl font-medium text-gray-800">Registration</h2>

    <form class="w-full" @submit.prevent="onSubmit">
      <div class="mb-4">
        <Label for="name" class="text-sm font-medium" :is-error="!!form.errors.name">
          Your Name
        </Label>
        <Input
            required
            id="name"
            v-model="form.name"
            placeholder="John Smith"
            name="name"
            :is-error="!!form.errors.name"
        />
        <InputErrorMessage :message="form.errors.email" />
      </div>
      <div class="mb-2">
        <Label for="email" class="text-sm font-medium" :is-error="!!form.errors.email">
          Email
        </Label>
        <Input
            required
            id="email"
            v-model="form.email"
            placeholder="example@gmail.com"
            name="email"
            type="email"
            :is-error="!!form.errors.email"
        />
        <InputErrorMessage :message="form.errors.email" />
      </div>

      <div class="mb-2">
        <Label for="password" class="text-sm font-medium" :is-error="!!form.errors.password">
          Password
        </Label>
        <Input
            required
            id="password"
            v-model="form.password"
            placeholder="*********"
            name="password"
            type="password"
            :is-error="!!form.errors.password"
        />
      </div>
      <div class="mb-2">
        <Label for="confirmation_password" class="text-sm font-medium" :is-error="!!form.errors.password_confirmation">
          Confirm Password
        </Label>
        <Input
            required
            id="password_confirmation"
            v-model="form.password_confirmation"
            placeholder="*********"
            name="password_confirmation"
            type="password"
            :is-error="!!form.errors.password_confirmation"
        />
      </div>

      <div class="mb-6 flex justify-between px-1 text-sm text-gray-500">
        <Link
            :href="route('auth.forgot.password')"
            class="hover:text-primary cursor-pointer"
        >
          Forgot password?
        </Link>
        <Link
            :href="route('auth.login')"
            class="hover:text-primary cursor-pointer"
        >
          Have an account?
        </Link>
      </div>

      <div class="flex justify-center">
        <Button
            class="w-full cursor-pointer text-base"
            type="submit"
            :disabled="form.hasErrors"
            :loading="form.processing"
        >
          Register
        </Button>
      </div>
    </form>
  </div>
</template>